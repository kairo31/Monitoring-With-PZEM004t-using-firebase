...
class BudgetEnergyEnv(Env):
    ...
    IDX_PREDIKSI_WATT = 12
    IDX_GAP_TO_TARGET = 14
...
def initialize_firebase() -> None:
    if not Path(PATH_JSON).exists():
        raise FileNotFoundError(f"File kredensial Firebase tidak ditemukan di: {PATH_JSON}")
...
if __name__ == "__main__":
    ...
    print(f"Creating RL Environment with target Rp {target_rp:,.2f} / month ...")
    env = BudgetEnergyEnv(rows, monthly_target_rp=target_rp)

    print("Load existing model or create new PPO agent...")
    if MODEL_OUTPUT_PATH.exists():
        print(f"Loading existing RL model from {MODEL_OUTPUT_PATH}")
        model = PPO.load(str(MODEL_OUTPUT_PATH), env=env)
    else:
        print("No existing model found. Creating new PPO model.")
        model = PPO("MlpPolicy", env, verbose=1)

    print(f"Start learning for {TOTAL_TIMESTEPS} timesteps...")
